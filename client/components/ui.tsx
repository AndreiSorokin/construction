 ...
 *  Общая для заявок и нарядов: поля шага (id/label/approverName/role/decision) совпадают у обеих моделей. */
export function StageTrack({ steps, currentIndex, status }: { steps: any[]; currentIndex: number; status: string }) {
  return (
      ...
      <div className="w-full max-w-sm rounded-2xl bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        {dlg.kind === 'prompt' && (
          <input autoFocus value={val} onChange={(e) => setVal(e.target.value)}
            className="mt-3 w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm text-stone-900" />
        )}
