...
import { createFuelSupplyRequest } from "@/lib/supply-requests-api";
...
export function FuelRequestModal({
  isOpen,
  object,
  onClose,
  onError,
  onSuccess,
}: FuelRequestModalProps) {
  useEffect(() => {
    if (!isOpen) {
      return;
    }
  ...
  async function submitFuelRequest(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const formElement = event.currentTarget;
    const form = new FormData(formElement);

    try {
      const request = await createFuelSupplyRequest({
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-h-[92vh] w-full max-w-xl flex-col rounded-lg bg-white shadow-xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-4 py-4 sm:px-5">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              {object.name}: укажите тип топлива и назначение заявки.
            </p>
          </div>
          <button
            aria-label="Закрыть"
        ...
        <form className="grid gap-4 px-4 py-4 sm:px-5" onSubmit={submitFuelRequest}>
          <label className="grid gap-1.5">
            ...
            <input
              ...
              name="fuelType"
              placeholder="Например: дизель, бензин АИ-92"
              required
            />
          </label>
          ...
          <label className="grid gap-1.5">
            ...
            <textarea
              ...
              name="purpose"
              placeholder="Например: заправка техники на объекте"
              required
            />
          </label>
